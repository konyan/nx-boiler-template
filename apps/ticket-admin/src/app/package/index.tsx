...
const PackageHeader = ['Name', 'Pricing', ' status', 'Action'];
...
const PackagePage = () => {
  ...
  const fetchPackageList = async () => {
    await ApiService.get('/admin/package?page=1&limit=10&orderBy=asc').then(
      (res) => setPackageList(res.data.data)
    );
  };
  const handleAddUser = () => {
    ...
    <div>
      ...
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
              {PackageHeader.map((n: string, idx) => (
          ...
          <TableBody>
            {packageList?.map((pc: PackageList, idx) => (
              <TableRow key={idx}>
                <TableBodyCell>{pc.name}</TableBodyCell>
                <TableBodyCell>{pc.price}</TableBodyCell>
                <TableBodyCell>{pc.description}</TableBodyCell>
                <TableBodyCell>
                  <Menu
                    as="div"
